...
public class CTelephoneInfo {
	...
	private void setComTelephoneInfo(){
	...
	public void setGTTelephoneInfo() {
		try {
			...
			int simId_2 = 1;
			if(obj==null){
				CTelephoneInfo.imeiSIM1 = getOperatorBySlot(mContext, "getDeviceId", simId_1);
				CTelephoneInfo.imeiSIM2 = getOperatorBySlot(mContext, "getDeviceId", simId_2);
			}else{
				Method md = cx.getMethod("getDeviceId", int.class);
				CTelephoneInfo.imeiSIM1 = (String) md.invoke(obj, simId_1);
				CTelephoneInfo.imeiSIM2 = (String) md.invoke(obj, simId_2);
			}
	...
	public String getImei(){
		...
		if (Constant.PLATFORM == TYPE_GT) {
			CTelephoneInfo.setGTTelephoneInfo();
		}
		if (TextUtils.isEmpty(CTelephoneInfo.imeiSIM1) && 
				TextUtils.isEmpty(CTelephoneInfo.imeiSIM2)) {
			CTelephoneInfo.setComTelephoneInfo();
		}
		
		if (!TextUtils.isEmpty(imeiSIM1)) {
			imeiBuffer.append(imeiSIM1) ;
		}
		if (!TextUtils.isEmpty(imeiSIM2)) {
			if (TextUtils.isEmpty(imeiBuffer.toString())) {
				imeiBuffer.append(imeiSIM2) ;
			}else {
				imeiBuffer.append( "," + imeiSIM2);
			}
		}
		return imeiBuffer.toString() ;
	}
}
